...
public class NettyMessageDecoder extends LengthFieldBasedFrameDecoder {
    ...
    public NettyMessageDecoder(int maxFrameLength, int lengthFieldOffset, int lengthFieldLength) throws IOException {
        ...
        this.nettyMarshallingDecoder = new NettyMarshallingDecoder();
    }
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf in) throws Exception {
        ...
        header.setType(MessageType.getMessageType(frame.readByte()));
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                attach.put(key, nettyMarshallingDecoder.decode(frame));
            }
            header.setAttachment(attach);
        }
        if (frame.readableBytes() > 4) {
            message.setBody(nettyMarshallingDecoder.decode(frame));
        }
